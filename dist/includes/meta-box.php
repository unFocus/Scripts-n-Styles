...
add_action(
	'add_meta_boxes', function() {
		...
		add_meta_box(
			...
			/**
			* Admin Action: 'add_meta_boxes'
			* Outputs the Meta Box. Only called on callback from add_meta_box()
			* during the add_meta_boxes action.
			*
		 ...
		 */
		add_filter(
			'tiny_mce_before_init', function( $init ) {
				global $post;
				$sns = get_post_meta( $post->ID, '_SnS', true );
				$styles = isset( $sns['styles'] ) ? $sns['styles'] : array();
...
add_action(
	'save_post', function( $post_id ) {
		...
		*/
		...
		$sns = is_array( $sns ) ? $sns : array(); // http://php.net/manual/en/migration71.incompatible.php#migration71.incompatible.empty-string-index-operator for explaination.
		$scripts = isset( $sns['scripts'] ) ? $sns['scripts'] : array();
		$styles  = isset( $sns['styles'] ) ? $sns['styles'] : array();
		...
		$shortcodes = array();
		$sns_shortcodes = isset( $_REQUEST['SnS_shortcodes'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['SnS_shortcodes'] ) ) : array();
...
function maybe_set_metabox( $o, $i, $p = 'SnS_' ) {
	...
	if ( empty( $_REQUEST[ $p . $i ] ) ) {
		...
		$o[ $i ] = sanitize_text_field( wp_unslash( [ $p . $i ] ) );
	}
	return $o;
}
