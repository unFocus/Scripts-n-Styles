...
add_action(
	'wp_ajax_sns_open_theme_panels', function () {
...
add_action(
	'wp_ajax_sns_tinymce_styles', function () {
...
// Ajax Saves.
add_action(
	'wp_ajax_sns_classes', function () {
		check_ajax_referer( BASENAME );
		if ( ! current_user_can( 'unfiltered_html' ) || ! current_user_can( 'edit_posts' ) ) {
		...
		if ( ! isset( $_REQUEST['classes_body'], $_REQUEST['classes_post'] ) ) {
			exit( 'Data missing.' );
		}
...
add_action(
	'wp_ajax_sns_scripts', function () {
		...
		if ( ! isset( $_REQUEST['scripts'], $_REQUEST['scripts_in_head'] ) ) {
			exit( 'Data incorrectly sent.' );
		}
...
add_action(
	'wp_ajax_sns_dropdown', function () {
		...
		if ( empty( $_REQUEST['format']['title'] ) ) {
			exit( 'Title is required.' );
		}
		if ( empty( $_REQUEST['format']['classes'] ) ) {
			exit( 'Classes is required.' );
		}
		if (
...
add_action(
	'wp_ajax_sns_delete_class', function () {
		check_ajax_referer( BASENAME );
		if ( ! current_user_can( 'unfiltered_html' ) || ! current_user_can( 'edit_posts' ) ) {
			exit( 'Insufficient Privileges.' );
		}
		...
		if ( isset( $styles['classes_mce'][ $title ] ) ) {
			unset( $styles['classes_mce'][ $title ] );
		} else {
			exit( 'No Format of that name.' );
		}
		...
		if ( ! isset( $styles['classes_mce'] ) ) {
			$styles['classes_mce'] = array( 'Empty' );
		}
...
add_action(
	'wp_ajax_sns_shortcodes', function () {
		check_ajax_referer( BASENAME );
		if ( ! current_user_can( 'unfiltered_html' ) || ! current_user_can( 'edit_posts' ) ) {
			exit( 'Insufficient Privileges.' );
		}
